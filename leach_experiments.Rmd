...
```{r salty metals Cu}
sort(saltLH$Curemove) #Seven zeros removals
#Set zeros to 1-max (0.05)
saltLH$Curemove[saltLH$Curemove==0] = 0.05

boxplot(Curemove~Salt*Metal,data=saltLH)

CuSmod <- lmer(logit(Curemove)~Salt*Metal+(1|Cell_no),data=saltLH)
CuSmod1 <- update(CuSmod,~.-Salt:Metal)
anova(CuSmod,CuSmod1) #Interaction is not signficant (p=0.83)
CuSmod2 <- update(CuSmod1,~.-Salt)
anova(CuSmod1,CuSmod2) #Salt is not significant (p=0.76)
CuSmod3 <- update(CuSmod1,~.-Metal)
anova(CuSmod1,CuSmod3) #Metal is significant (p<0.001)

#CuSmod <- lme(logit(Curemove)~Salt*Metal,random=~1|Cell_no,data=saltLH)
#anova(CuSmod)

boxplot(log(Cu_conc)~Salt*Metal,data=saltN)
```
...
add1(Pbnull,scope=Pbfull,test="Chisq")
Pb1 <- update(Pbnull,~.+Cell_no)

add1(Pb1,scope=Pbfull,test="Chisq")
Pb2 <- update(Pb1,~.+Salt)

add1(Pb2,scope=Pbfull,test="Chisq")
Pb3 <- update(Pb2,~.+Metal)
...
add1(Pb3,scope=Pbfull,test="Chisq")
Pb4 <- update(Pb3,~.+Location)
#Same here, add p < 0.1

add1(Pb4,scope=Pbfull,test="Chisq")
Pb5 <- update(Pb4,~.+Cell_no:Location)
#This is where that became important since adding location was required to add 
#the Cell x location interaction

add1(Pb5,scope=Pbfull,test="Chisq")
#Nothing else needs to be added

drop1(Pb5,test="Chisq")
#Use drop1 to double check that nothing added is now insignificant
#Here everything looks good and should stay. Pb5 is the final model
...
saltleach$Pb_pred <- fitted(Pb5) #Predicted values from the final model
...
sigma_Pb <- Pb5$scale
(p0 <- 1-pnorm((mu_Pb-log(10))/Pb5$scale))
