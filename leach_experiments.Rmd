...
chemmean <- aggregate(soilchem[soilchem$Depth=="0 to 10",c('OM','BD','d50','logOM','logBD','logd50',
...
CuHresult[order(CuHresult$Pval),] #Add log OM
...
for(i in 1:length(candvar)){
  Cudata <- cbind(leachH[,c('Cell_no','Curemove','Retrofit','logOM')],leachH[,loc[i]])
  Cudata <- Cudata[complete.cases(Cudata),]
  
  MCu1 <- lmer(logit(Curemove)~Retrofit+(1|Cell_no),data=Cudata,REML=F)
  MCu2 <- lmer(logit(Curemove)~Retrofit+Cudata[[5]]+(1|Cell_no),data=Cudata,REML=F)
  
  CuHresult$Pval[i] <- anova(MCu2,MCu1)$P[2]
}

CuHresult[order(CuHresult$Pval),] #Nothing else to add
...
newOM <- quantile(leachH$logOM,na.rm=T,0.9)

inv.logit(predict(CuH,newdata=list(Retrofit="yes",logOM=newOM),re.form=~0))
inv.logit(predict(CuH,newdata=list(Retrofit="no",logOM=newOM),re.form=~0))
```

A large proportion of Cu was removed from high metal stormwater, but the removal efficiency differed between retrofit and new build biorentention cell soils (p=0.002). Retrofit cells only removed 77% of added Cu but new builds removed 89%. Soils with greater OM have less metal removal. In new builds, low OM (10%) removed 92% of Cu but high OM (90%) removed 85%. 
...
sort(leachH$Znremove) #17 values of zero removal
...
anova(ZnH1,ZnH2)
#Interaction is not signficant (p=0.10)
...
Zn removal was very efficient with a lot of soils removing almost all of the 441 µg/L of added Zn. The removal rates were modified by construction type (retrofit vs. new build) and location of sampling. Retrofits yielded less efficient metal removal than new builds and soils near inflow points removed more metal than drains. Log odds decreased -2.2 when retrofitted and log odds decreased -1.18 when soils were near the drain.

```{r Low metal stormwater}
plot(leachL$Znremove,ylim=c(0,1))
sort(leachL$Znremove) #Nine zeros removals
