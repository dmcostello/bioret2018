...
#pdf(file="cellmetal.pdf",height=5,width=4)

par(mfrow=c(3,1),oma=c(3,5,0,1),mar=c(0,0,2,0))
plot(Zn~Age,data=fullsoil,log="y",yaxt="n",ylim=c(10,1000),xaxt="n")
logaxis(1,3,2)
axis(1,labels=F)
mtext(expression("Soil Zn (mg kg"^-1*")"),2,line=3,cex=0.8)

par(mar=c(1,0,1,0))
plot(Pb~Age,data=fullsoil,log="y",yaxt="n",ylim=c(5,1000),xaxt="n")
logaxis(1,3,2)
axis(1,labels=F)
mtext(expression("Soil Pb (mg kg"^-1*")"),2,line=3,cex=0.8)

par(mar=c(2,0,0,0))
plot(Cu~Age,data=fullsoil,log="y",yaxt="n",ylim=c(5,1000))
logaxis(0,3,2)
